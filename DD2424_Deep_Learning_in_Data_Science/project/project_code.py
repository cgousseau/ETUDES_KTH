...
Original file is located at
    https://colab.research.google.com/drive/1bo91oPO9ddBBHGdZJW89C2xb2PkXNECt
"""
...
def run_model(data, image_dim, label_count, depth):
  ...
  with graph.as_default():
    ...
    current, features = block(current, layers, 16, 16, is_training, keep_prob)
    current = batch_activ_conv(current, features, features, 1, is_training, keep_prob)
    current = avg_pool(current, 2)
    current, features = block(current, layers, features, 16, is_training, keep_prob)
    current = batch_activ_conv(current, features, features, 1, is_training, keep_prob)
    current = avg_pool(current, 2)
    current, features = block(current, layers, features, 16, is_training, keep_prob)
  ...
  with tf.Session(graph=graph) as session:
    ...
    for epoch in range(1, 1+300):
      if epoch == 20: learning_rate = 0.05
      if epoch == 25: learning_rate = 0.01
      if epoch == 35: learning_rate = 0.003
      if epoch == 45: learning_rate = 0.001
      for batch_idx in range(batch_count):
        xs_, ys_ = batches_data[batch_idx], batches_labels[batch_idx]
        batch_res = session.run([ train_step, cross_entropy, accuracy ],feed_dict = { xs: xs_, ys: ys_, lr: learning_rate, is_training: True, keep_prob: 0.8 })
